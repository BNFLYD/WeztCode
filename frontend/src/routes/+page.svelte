    ...
    <div class="flex w-full justify-between px-4 py-2">
      {#each sections as section}
        <button
          ...
          section.id
            ? 'relative text-back bg-accent-detail/95 after:absolute after:inset-0 after:pointer-events-none after:bg-[radial-gradient(circle_at_center,rgba(var(--glow-rgb),0.25)_0%,rgba(var(--glow-rgb),0.08)_60%,transparent_100%)] after:mix-blend-overlay after:opacity-80'
            : 'text-back-deep'}"
